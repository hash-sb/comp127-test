---
repo: https://github.com/mac-comp127/weather-display
---
...
![Completed user interface with 4 widgets](images/completed-ui.png)

### Learning Goals

This assignment focuses on graphics, object modeling, polymorphism, and testing. Its goals include:

- Writing code that works with real, live data
...
We have used the `owmjapis` and `org.json` packages (both of which are open source) to make it easier for you to use OpenWeather data. You do not need to know how these packages work, but if you would like more information about them, see [this OWN JAPIs repository](https://bitbucket.org/akapribot/owm-japis) and [the Java JSON parser package](https://github.com/stleary/JSON-java).

The original version of this activity was by Bret Jackson, with heavy contributions from Daniel Kluver and Paul Cantrell.


## Task 0: Get an API Key
...
Inside `res/`, create a new file named `weather-display.properties`. <highlight>Make **sure** you have the correct location!️</highlight> In VS Code’s UI, it is easy to accidentally create the file inside of `res/condition-icons/`. Make sure you create the file just inside of `res/`.
...
![Initial UI state](images/initial-ui-state.png)
...
Note that <highlight>this isn't a `JUnit` test class!</highlight> There won't be any icons on the left side that run unit testing cases. Just run this class as usual: from the Run and Debug pane, with F5, clicking the arrow in the upper right, or similar.
...
## Task 2: Fix `TemperatureWidget`

We want `TemperatureWidget` to look something like this:

![Temperature widget working in various states](images/temperature-widget-completed.png)

…but right now, it looks like this:

![Temperature widget showing bugs in various states](images/temperature-widget-starting-point.png)
...
## Task 3: Complete `ForecastWidget`

Make the `ForecastWidget` look like this:

![Forecast widget working](images/forecast-widget-completed.png)
...
To learn about what kinds of graphics you can create, take a look at the [Comp 127 kilt-graphics javadoc](https://mac-comp127.github.io/kilt-graphics/).
